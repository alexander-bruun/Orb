...
<root xmlns="urn:schemas-upnp-org:device-1-0"
  ...
  <device>
    ...
    <manufacturer>Orb</manufacturer>
    <manufacturerURL>https://github.com/alexander-bruun/orb</manufacturerURL>
    <modelDescription>Orb Music Server</modelDescription>
...
</root>`, xmlEscape(s.name), xmlEscape(s.udn))
}

// handleContentDirectoryDesc serves the Content Directory SCPD.
func (s *Server) handleContentDirectoryDesc(w http.ResponseWriter, _ *http.Request) {
	w.Header().Set("Content-Type", "text/xml; charset=utf-8")
...
// handleConnectionManagerDesc serves the Connection Manager SCPD.
func (s *Server) handleConnectionManagerDesc(w http.ResponseWriter, _ *http.Request) {
	w.Header().Set("Content-Type", "text/xml; charset=utf-8")
	_, _ = w.Write([]byte(`<?xml version="1.0" encoding="UTF-8"?>
<scpd xmlns="urn:schemas-upnp-org:service-1-0">
  <specVersion><major>1</major><minor>0</minor></specVersion>
...
// replaceAll is a simple string replacer to avoid importing strings for a trivial op.
func replaceAll(s, old, new string) string {
	// Use strings from the import in dlna.go; this is just a helper.
	result := ""
	for {
		i := indexOf(s, old)
		if i < 0 {
