<script lang="ts">
  ...
  import { isOffline, checkConnectivity } from '$lib/stores/offline';
  import { downloads, restoreDownloads } from '$lib/stores/downloads';
...
{:else if offline && !hasDownloads}
  <div class="error-page">
    ...
    <p class="sub">The server can't be reached and you have no downloaded tracks.</p>
    <p class="hint">Download music while connected to enable offline playback.</p>
    <button class="btn-retry" on:click={() => { checking = true; checkConnectivity().then(down => { if (!down) goto('/'); else checking = false; }); }}>
      Retry Connection
    </button>
  </div>
{:else}
  <div class="error-page">
    ...
    <p class="sub">{$page.status} — {$page.error?.message ?? 'Unknown error'}</p>
    <div class="actions">
      <button class="btn-retry" on:click={() => goto('/')}>Go Home</button>
      <button class="btn-retry" on:click={() => location.reload()}>Reload</button>
    </div>
  </div>
{/if}
