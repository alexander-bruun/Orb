<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (track.featured_artists && track.featured_artists.length) {
      featuredArtists = track.featured_artists.map((a) => ({
        id: a.id,
        name: a.name,
      }));
      ...
      featuredArtists = await Promise.all(
        track.featured_artist_ids.map(async (id) => ({
          id,
          name: await getArtistName(id),
        })),
      );
    }
  });
...
>
  ...
  {#if showCover}
    <div class="cover-thumb">
      {#if track.album_id}
        <img
          src="{getApiBase()}/covers/{track.album_id}"
          alt=""
          loading="lazy"
        />
      {:else}
  ...
  <div class="track-info">
    ...
    {#if mainArtist || featuredArtists.length}
      <span class="meta">
        {#if mainArtist}
          {#if mainArtist.id}
            <a class="artist-link" href="/artists/{mainArtist.id}"
              >{mainArtist.name}</a
            >
          {:else}
  ...
  <span class="row-actions">
    <button
      ...
      aria-label={$favorites.has(track.id)
        ? "Remove from favorites"
        : "Add to favorites"}
      title={$favorites.has(track.id)
        ? "Remove from favorites"
        : "Add to favorites"}
    >
      <svg
  ...
  {#if dlStatus === "done"}
    ...
    >
      <svg
        ...
        height="14"
        fill="currentColor"
        ><path d="M12 16l-5-5h3V4h4v7h3l-5 5zm-7 2h14v2H5v-2z" /></svg
      >
      <svg
        class="dl-x-icon"
        xmlns="http://www.w3.org/2000/svg"
...
<style>
  ...
  .dl-done-btn .dl-x-icon {
    display: none;
    color: var(--text-muted);
  }
  .dl-done-btn:hover .dl-dl-icon {
    display: none;
  }
  .dl-done-btn:hover .dl-x-icon {
    display: block;
  }
  .dl-done-btn:hover {
    opacity: 1;
