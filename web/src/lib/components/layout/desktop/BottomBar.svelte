    ...
    for (let i = 1; i <= steps; i++) {
      ...
      const wy =
        ...
        SEEK_WAVE_AMP *
          waveAmp *
          env *
          Math.sin((x / SEEK_WAVE_LEN) * Math.PI * 2 + wavePhase);
...
<svelte:window
  on:click={(e) => {
    ...
    if (sleepMenuOpen && !t.closest?.(".sleep-timer-wrap"))
      sleepMenuOpen = false;
    if (narrowMenuOpen && !t.closest?.(".narrow-menu-wrap"))
      narrowMenuOpen = false;
    if (volumePopupOpen && !t.closest?.(".volume-popup-wrap"))
    ...
    {#if $currentTrack}
      ...
      <div class="track-meta" class:full-width={$expanded}>
        {#if $bottomBarSecondary === "album"}
          {#if $currentAlbum}
            <div class="album-title">
              <a href="/library/albums/{$currentAlbum.id}" class="album-link"
                >{$currentAlbum.title}</a
              >
            </div>
          {:else if $currentTrack.album_name}
            <div class="album-title">
              {#if $currentTrack.album_id}
                <a
                  href="/library/albums/{$currentTrack.album_id}"
                  class="album-link">{$currentTrack.album_name}</a
                >
              {:else}
        ...
        {:else if $currentTrack.artist_id}
          <div class="album-title">
            <a href="/artists/{$currentTrack.artist_id}" class="album-link"
              >{$currentTrack.artist_name ?? $currentTrack.artist_id}</a
            >
          </div>
  ...
  <div class="playback-section">
    <div class="controls">
          ...
          <div class="narrow-menu" role="menu">
            ...
            >
              <svg
                ...
                viewBox="0 0 24 24"
                fill="currentColor"
                ><polygon points="19,4 9,12 19,20" /><rect
                  x="5"
                  y="4"
                  width="2.5"
                  height="16"
            ...
            <button
              ...
              aria-label="Shuffle"
              title={$shuffle && $smartShuffleEnabled
                ? "Smart Shuffle on"
                : "Shuffle"}
            >
              <svg
      ...
      <button class="ctrl-btn" on:click={next} aria-label="Next">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"
          ><polygon points="5,4 15,12 5,20" /><rect
            x="16"
            y="4"
            width="2.5"
            height="16"
            ...
            >
              ...
              {#if (seekBarWidth * $bufferedPct) / 100 > progressX}
                <line
                  x1={progressX}
                  y1="2"
                  x2={(seekBarWidth * $bufferedPct) / 100}
                  y2="2"
    ...
    <div class="right-controls">
      ...
      {#if $visualizerButtonEnabled}
        <button
          ...
          aria-label="{$visualizerStore.visible
            ? 'Hide'
      ...
      {#if $musicSleepTimerEnabled}
        <div class="sleep-timer-wrap">
          <button
            ...
            title={$musicSleepPreset !== null
              ? `Sleep timer: ${formatSleepRemaining($musicSleepMsRemaining)}`
              : "Sleep timer"}
          >
            <svg
              width="14"
            ...
            {#if $musicSleepPreset !== null && $musicSleepMsRemaining !== 0}
              <span class="sleep-badge"
                >{formatSleepRemaining($musicSleepMsRemaining)}</span
              >
            {/if}
          </button>
...
<style>
  ...
  .seek-bar-wrap:hover .seek-input::-webkit-slider-thumb {
    opacity: 1;
  }
  .seek-bar-wrap:hover .seek-input::-moz-range-thumb {
    opacity: 1;
  }
  .seek-playhead {
    transition: opacity 0.15s;
  }
  .seek-bar-wrap:hover .seek-playhead {
    opacity: 0;
  }
