<script lang="ts">
  ...
  $: dropdownVisible =
    searchFocused && query.trim().length > 0 && quickResults !== null;
  $: hasQuickResults =
    quickResults &&
  ...
  function handleSearchInput() {
    ...
    searchDebounce = setTimeout(async () => {
      try {
        const res = await libApi.search(query.trim(), {
          ...
          {#each quickResults.albums.slice(0, 3) as album (album.id)}
            ...
            >
              ...
              <div class="qd-text">
                ...
                {#if album.artist_name}<span class="qd-sub"
                    >{album.artist_name}</span
                  >{/if}
              </div>
              {#if album.release_year}<span class="qd-meta"
                  >{album.release_year}</span
                >{/if}
            </button>
          {/each}
        {/if}
          ...
          {#each quickResults.tracks.slice(0, 5) as track (track.id)}
            ...
            >
              ...
              <div class="qd-text">
                ...
                {#if track.artist_name}<span class="qd-sub"
