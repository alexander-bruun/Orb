        ...
        {#if sinkIdSupported && $audioOutputDevices.length > 0}
          ...
          {#each $audioOutputDevices as out (out.deviceId)}
            <button
              ...
              on:click={() => { setAudioOutput(out.deviceId); open = false; }}
            >
              <div class="fs-device-left">
...
<style>
  ...
  .fs-device-item:active, .fs-device-item:hover { background: rgba(255, 255, 255, 0.07); }
  .fs-device-item.is-active { color: #fff; }
  .fs-device-left { display: flex; align-items: center; gap: 10px; min-width: 0; }
  .fs-device-dot {
  ...
  .fs-device-info { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .fs-device-name {
    font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    display: flex; align-items: center; gap: 6px;
  }
  .fs-this-badge { font-size: 10px; font-weight: 400; color: rgba(255, 255, 255, 0.4); }
  .fs-device-track {
    font-size: 0.75rem; color: rgba(255, 255, 255, 0.4);
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
  .fs-transfer-hint {
    font-size: 11px; color: var(--accent, #1db954); flex-shrink: 0; font-weight: 500;
  }
</style>
