<script lang="ts">
  ...
  const numericFields = new Set<SmartPlaylistField>([
    "year",
    "bit_depth",
    "duration_ms",
    "play_count",
    ...
    suggestionData = {
      genre:
        genres.status === "fulfilled"
          ? (genres.value ?? []).map((g: any) => g.name)
          : [],
      artist:
        artists.status === "fulfilled"
          ? (artists.value ?? []).map((a: any) => a.name)
          : [],
      album:
        albums.status === "fulfilled"
          ? (albums.value?.items ?? []).map((a: any) => a.title)
          : [],
      format: FORMATS,
    };
  }
  ...
  <div class="page">
        ...
        {#each rules as rule, i (i)}
          <div class="rule-row">
            ...
            <select
              ...
              value={rule.field}
              on:change={(e) =>
                onFieldChange(i, e.currentTarget.value as SmartPlaylistField)}
            >
              {#each FIELDS as f}
    ...
    <section class="section">
      <div class="section-header">
        ...
        {#if pl.last_built_at}
          <span class="muted"
            >Last built {new Date(pl.last_built_at).toLocaleString()}</span
          >
        {/if}
      </div>
      {#if tracksLoading}
      ...
      {:else if tracks.length === 0}
        <p class="muted">
          No tracks match the current rules. Add rules and save to refresh.
        </p>
      {:else}
