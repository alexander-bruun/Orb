<script lang="ts">
  ...
  $: isDownloaded =
    book && chapters.length > 0
      ? isAudiobookDownloaded(book.id, chapters)
      : false;
  ...
  function startEdit(field: EditField) {
    ...
    switch (field) {
      case "title":
        ...
        break;
      case "author":
        editValue = book.author_name ?? "";
        break;
      case "description":
        editValue = book.description ?? "";
        break;
      case "series":
        editValue = book.series ?? "";
        break;
      case "series_index":
        editValue = book.series_index != null ? String(book.series_index) : "";
        break;
      case "published_year":
        editValue =
  ...
  async function commitEdit() {
    ...
    try {
      ...
      if (editingField === "title") book = { ...book, title: body.title };
        ...
        book = { ...book, author_name: body.author_name ?? undefined };
      else if (editingField === "description")
        book = { ...book, description: body.description ?? undefined };
      else if (editingField === "series")
        book = { ...book, series: body.series ?? undefined };
      else if (editingField === "series_index")
        book = { ...book, series_index: body.series_index ?? undefined };
      else if (editingField === "published_year")
        book = { ...book, published_year: body.published_year ?? undefined };
      editingField = null;
    } catch (e) {
  ...
  function handleChapterPlay(ch: AudiobookChapter) {
    ...
    if (isActive) {
      ...
      const isSameChapter =
        $currentAudiobook?.id === id &&
        $abPositionMs >= ch.start_ms &&
        $abPositionMs < ch.end_ms;
      if (isSameChapter) {
        toggleABPlayPause();
      } else {
        jumpToChapter(ch);
  ...
  async function loadBook(bookId: string) {
    ...
    try {
      ...
      if (book.series) {
        abApi
          .listBySeries(book.series)
          .then((r) => {
            seriesBooks = (r.audiobooks ?? []).filter((b) => b.id !== bookId);
          })
          .catch(() => {});
      }
    } catch (e: unknown) {
      ...
      <div class="meta-col">
        ...
        <p class="series-label">
          {#if editingField === "series"}
            <input
              class="inline-input series-input"
          ...
          {:else if book.series}
            ...
            >
              {seriesName}{book.series_index != null
        ...
        <p class="author">
          ...
          {:else if isAdmin}
            <button
              ...
              aria-label={book.author_name
                ? `Edit author ${book.author_name}`
                : "Add author"}
          ...
          {:else if book.author_name}
            <span class="author-text" title={book.author_name}
            ...
            <div class="progress-track">
              <div
                ...
                style="width: {Math.min(
                  100,
                  (progress.position_ms / book.duration_ms) * 100,
                )}%"
              ></div>
            </div>
            <span class="resume-label">
              {#if progress.completed}Completed{:else}{fmtMs(
                  progress.position_ms,
                )} / {fmtMs(book.duration_ms)}{/if}
            </span>
          </div>
        {/if}
        ...
        <div class="actions">
          <button class="btn-play" on:click={handlePlay}>
            ...
            {:else if isActive || (progress && progress.position_ms > 0 && !progress.completed)}
              <svg
                ...
                viewBox="0 0 24 24"
                fill="currentColor"
                aria-hidden="true"><path d="M5 3l14 9-14 9V3z" /></svg
              >Resume
            {:else}
              <svg
                width="18"
          ...
          {#if progress && progress.position_ms > 0 && !progress.completed && !isActive}
            <button
              ...
              on:click={() => book && playAudiobook(book, 0)}>Start over</button
            >
          {/if}

          {#if isDownloaded}
            <button
              class="btn-downloaded"
              on:click={handleDeleteDownload}
              title="Delete downloaded audiobook"
            >
              <svg
                width="16"
                height="16"
          ...
          {:else if !isDownloading}
            <button
              ...
              title="Download audiobook for offline use"
            >
              <svg
                width="16"
                height="16"
          ...
          {#if isAdmin}
            <button
              ...
              disabled={refreshing}
              title="Refresh metadata from Open Library"
            >
              {#if refreshing}<svg
                  class="spin-sm"
                  width="12"
            ...
            <button
              ...
              disabled={scanning}
              title="Re-scan audiobook files from disk"
            >
              {#if scanning}<svg
                  class="spin-sm"
                  width="12"
        ...
        <div class="description-wrap">
          ...
          {:else if isAdmin}
            ...
            >
              {#if book.description}
                <p class="description">
                  {book.description}<span class="edit-hint">✎</span>
                </p>
              {:else}
      ...
      <section class="section">
        <div class="series-header">
          ...
          <button
            ...
            aria-label={`View all books in ${book!.series}`}>View all</button
          >
        </div>
        <div class="carousel">
          {#each seriesBooks as sb (sb.id)}
            ...
            >
              ...
              {#if sb.series_index != null}<span class="carousel-idx"
                  >Book {sb.series_index}</span
                >{/if}
            </div>
          {/each}
        </div>
...
<style>
  ...
  .series-trigger:not(.editable):hover {
