<script lang="ts">
  ...
  import CrossfadeOverlapEditor from "$lib/components/ui/CrossfadeOverlapEditor.svelte";
  ...
  import {
    seekBarMode,
    visualizerButtonEnabled,
    bottomBarSecondary,
    listenAlongEnabled,
    autoDownloadFavorites,
    musicSleepTimerEnabled,
    audiobookSleepTimerEnabled,
  ...
  function blurEmailNew() {
    emailNewError = validateEmail(emailNew);
  }
  function blurEmailPw() {
  ...
  async function startTotpSetup() {
    ...
    try {
      ...
      totpQrDataUrl = await QRCode.toDataURL(res.otpauth_url, {
        width: 200,
        margin: 1,
      });
      totpCode = "";
  ...
  async function confirmTotpEnable() {
    if (!totpCode) {
      totpError = "Enter the 6-digit code from your app.";
      return;
    }
    totpError = "";
    totpLoading = true;
    try {
      const res = await apiFetch<{ backup_codes: string[] }>(
        "/auth/totp/enable",
        {
          method: "POST",
  ...
  async function disableTotp() {
    if (!totpDisablePassword || !totpDisableCode) {
      totpError = "Password and code required.";
      return;
    }
    totpError = "";
    totpLoading = true;
  ...
  async function regenBackupCodes() {
    if (!totpRegenCode) {
      totpRegenError = "Enter your authenticator code.";
      return;
    }
    totpRegenError = "";
  ...
  async function loadEmailConfig() {
    try {
      const res = await apiFetch<{ verification_enabled: boolean }>(
        "/auth/email-config",
      );
  ...
  function matchPreset(
    ...
    for (const p of SQ_PRESETS) {
      if (p.bitrate === b && p.sampleRate === s && p.bitDepth === d)
        return p.id;
    }
    return "custom";
  }
  ...
  $: wifiPresetId = matchPreset(
    sqWifiMaxBitrate,
    sqWifiMaxSampleRate,
    sqWifiMaxBitDepth,
  );
  $: mobilePresetId = matchPreset(
    sqMobileMaxBitrate,
    sqMobileMaxSampleRate,
    sqMobileMaxBitDepth,
  );
  ...
  function applyPreset(
    tier: "any" | "wifi" | "mobile",
    p: (typeof SQ_PRESETS)[number],
  ) {
    ...
    if (tier === "any") {
      ...
      sqWifiMaxBitrate = b;
      sqWifiMaxSampleRate = s;
      sqWifiMaxBitDepth = d;
    } else {
      sqMobileMaxBitrate = b;
      sqMobileMaxSampleRate = s;
      sqMobileMaxBitDepth = d;
    }
  }
  ...
  async function loadStreamingPrefs() {
    ...
    try {
      ...
      sqMaxBitrate =
        res.max_bitrate_kbps != null ? String(res.max_bitrate_kbps) : "";
      sqMaxSampleRate =
        res.max_sample_rate != null ? String(res.max_sample_rate) : "";
      sqMaxBitDepth =
        res.max_bit_depth != null ? String(res.max_bit_depth) : "";
      sqTranscodeFormat = res.transcode_format ?? null;
      sqWifiMaxBitrate =
        res.wifi_max_bitrate_kbps != null
          ? String(res.wifi_max_bitrate_kbps)
          : "";
      sqWifiMaxSampleRate =
        res.wifi_max_sample_rate != null
          ? String(res.wifi_max_sample_rate)
          : "";
      sqWifiMaxBitDepth =
        res.wifi_max_bit_depth != null ? String(res.wifi_max_bit_depth) : "";
      sqWifiTranscodeFormat = res.wifi_transcode_format ?? null;
      sqMobileMaxBitrate =
        res.mobile_max_bitrate_kbps != null
          ? String(res.mobile_max_bitrate_kbps)
          : "";
      sqMobileMaxSampleRate =
        res.mobile_max_sample_rate != null
          ? String(res.mobile_max_sample_rate)
          : "";
      sqMobileMaxBitDepth =
        res.mobile_max_bit_depth != null
          ? String(res.mobile_max_bit_depth)
          : "";
  ...
  async function handleCastClick() {
    ...
    if ($castState === "connected") {
      ...
      try {
        ...
        if (err?.code !== "cancel") {
          castError =
            "Could not connect to Cast device. Make sure you are on the same network.";
        }
      }
    }
  }
  ...
  async function connectLastFM() {
    if (!lfmUser || !lfmPass) {
      lfmError = "Username and password required.";
      return;
    }
    lfmError = "";
    lfmLoading = true;
    try {
      const res = await apiFetch<{ username: string }>(
        "/user/scrobble-settings/lastfm/connect",
        {
          method: "POST",
          body: JSON.stringify({ username: lfmUser, password: lfmPass }),
        },
      );
      scrobble = {
        ...scrobble,
        lastfm_connected: true,
        lastfm_username: res.username,
        lastfm_enabled: true,
      };
      lfmUser = "";
      lfmPass = "";
  ...
  async function toggleScrobble(service: "lastfm" | "lb", enabled: boolean) {
    ...
    try {
      ...
      if (service === "lastfm")
        scrobble = { ...scrobble, lastfm_enabled: enabled };
  ...
  $: allDoneEntries = [...$downloads.values()].filter(
    (d) => d.status === "done",
  );
  $: activeEntries = [...$downloads.values()].filter(
    (d) => d.status === "downloading",
  );
  $: errorEntries = [...$downloads.values()].filter(
    (d) => d.status === "error",
  );
  ...
  onMount(() => {
    ...
    const sections = Array.from(
      document.querySelectorAll<HTMLElement>(".page > section[id]"),
    );
    ...
    function updateActiveSection() {
      ...
      for (const section of sections) {
        if (section.getBoundingClientRect().top + scrollTop <= threshold)
          current = section;
      }
...
<div class="settings-shell">
  <nav class="settings-nav">
    ...
    <a
      class="settings-nav-link"
      class:active={activeSection === "eq"}
      href="#eq">Equalizer</a
    >
    <a
      class="settings-nav-link"
      class:active={activeSection === "playback"}
      href="#playback">Playback</a
    >
    <a
      class="settings-nav-link"
      class:active={activeSection === "sleep"}
      href="#sleep">Sleep Timer</a
    >
    <a
      class="settings-nav-link"
      class:active={activeSection === "scrobbling"}
      href="#scrobbling">Scrobbling</a
    >
    <a
      class="settings-nav-link"
  ...
  <div class="page">
        ...
        >
          <div class="setting-info">
            ...
            <span class="setting-desc"
              >The base URL of your Orb server (e.g.
              https://orb.example.com/api)</span
            >
          </div>
    ...
    <section id="profile" class="card">
        ...
        <div class="field">
          ...
          <div class="field-value field-value--row">
            ...
            {#if $authStore.user?.email_verified}
              <span class="badge badge--verified" title="Email verified"
                >Verified</span
              >
            {:else if smtpConfigured}
              <span class="badge badge--unverified" title="Email not verified"
                >Unverified</span
              >
            {/if}
          </div>
      ...
      {#if !$authStore.user?.email_verified && smtpConfigured}
        <div class="verify-banner">
          <span class="verify-banner__text"
            >Your email address has not been verified.</span
          >
          {#if resendSuccess}
            <span class="msg msg--ok" style="margin:0"
          ...
          {/if}
          {#if resendError}<span class="msg msg--error" style="margin:0"
              >{resendError}</span
            >{/if}
        </div>
      {/if}
      ...
      <div
        class="setting-row"
        style="border-top:none;margin-top:16px;padding-top:0;flex-direction:column;align-items:stretch;gap:12px;"
      >
        <div class="setting-row" style="border-top:none;padding-top:0">
          <div class="setting-info">
            <span class="setting-name">Make profile public</span>
            <span class="setting-desc"
              >Let others find and view your profile, activity, and public
              playlists.</span
            >
          </div>
          <button
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
          ...
          {#if profilePublic && $authStore.user}
            <a
              ...
              class="btn-ghost btn--sm">View profile →</a
            >
          {/if}
          {#if profileSaveMsg}<span class="msg msg--ok">{profileSaveMsg}</span
            >{/if}
          {#if profileSaveError}<span class="msg msg--error"
              >{profileSaveError}</span
            >{/if}
        </div>
      </div>
    </section>
      ...
      <div class="form-grid">
        ...
        <div class="field-col">
          <input
            ...
            disabled={pwLoading}
          />
          {#if pwCurrentError}<span class="field-error">{pwCurrentError}</span
            >{/if}
        </div>
        ...
        <div class="field-col">
          <input
            ...
            disabled={pwLoading}
          />
          {#if pwConfirmError}<span class="field-error">{pwConfirmError}</span
            >{/if}
        </div>
      </div>
    ...
    <section class="card">
      ...
      <div class="form-grid">
        ...
        <div class="field-col">
          ...
          />
          {#if emailNewError}<span class="field-error">{emailNewError}</span
            >{/if}
        </div>
    ...
    <section class="card">
      ...
      {#if totpStep === "idle"}
        <div class="setting-row" style="border-top:none;padding-top:0">
          <div class="setting-info">
            ...
            <span class="setting-desc">
              {totpEnabled
                ? "Enabled — your account requires a code on login."
        ...
        {#if totpEnabled}
          <div class="setting-row">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Generate new backup codes (current ones will be invalidated).</span
              >
            </div>
          </div>
          <div class="form-grid" style="margin-top:-4px">
            <label class="form-label" for="totp-regen-code"
          ...
          {#if totpRegenError}<p class="msg msg--error">
              {totpRegenError}
            </p>{/if}
          {#if totpRegenSuccess}
            <p class="msg msg--ok">
              New backup codes generated — save them now, they won't be shown
              again.
            </p>
            <div class="backup-grid">
              {#each totpRegenCodes as code}<code class="backup-code"
                  >{code}</code
                >{/each}
            </div>
          {/if}
      ...
      {:else if totpStep === "setup"}
        ...
        {#if totpQrDataUrl}
          <div class="qr-wrap">
            <img src={totpQrDataUrl} alt="TOTP QR code" class="qr-img" />
          </div>
        {/if}
        ...
        <div class="form-grid">
          <label class="form-label" for="totp-confirm-code"
        ...
        <div class="btn-row">
          ...
          <button
            class="btn-ghost"
            on:click={closeTotpSetup}
            disabled={totpLoading}>Cancel</button
          >
        </div>
      {:else if totpStep === "backup-codes"}
        <p class="totp-hint">
          <strong>2FA enabled!</strong> Save these backup codes somewhere safe. Each
          code can be used once to sign in if you lose access to your authenticator
          app. They won't be shown again.
        </p>
        <div class="backup-grid">
          {#each totpBackupCodes as code}<code class="backup-code">{code}</code
            >{/each}
        </div>
        <div>
      ...
      {:else if totpStep === "disable"}
        ...
        <div class="form-grid">
          ...
          <input
            ...
            disabled={totpLoading}
          />
          <label class="form-label" for="totp-dis-code"
            >Authenticator code</label
          >
          <input
            id="totp-dis-code"
    ...
    <section id="streaming" class="card">
      ...
      <p class="sq-hint">
        Server-enforced quality limits. Set a different level per connection
        type to save mobile data while keeping full quality at home.
      </p>
      ...
      {:else}
        ...
        {#if sqTab === "any"}
          <div class="sq-presets">
            {#each SQ_PRESETS as p (p.id)}
              <button
                class="sq-preset"
                class:active={anyPresetId === p.id}
                on:click={() => applyPreset("any", p)}
                disabled={sqSaving}
              >
          ...
          <button
            class="sq-custom-btn"
            class:open={sqAnyShowCustom || anyPresetId === "custom"}
            on:click={() => toggleCustom("any")}
            disabled={sqSaving}
          >
            <svg
              width="13"
              ...
              viewBox="0 0 24 24"
            >
              <line x1="4" y1="6" x2="20" y2="6" /><circle
                cx="8"
                cy="6"
                r="2"
                fill="currentColor"
                stroke="none"
              />
              <line x1="4" y1="12" x2="20" y2="12" /><circle
                cx="16"
                cy="12"
                r="2"
                fill="currentColor"
                stroke="none"
              />
              <line x1="4" y1="18" x2="20" y2="18" /><circle
                cx="10"
                cy="18"
                r="2"
                fill="currentColor"
            ...
            <div class="sq-custom-form">
              <div class="sq-custom-row">
                <div class="sq-custom-info">
                  <span class="sq-custom-label"
                    >Max bitrate <code>kbps</code></span
                  >
                  <span class="sq-custom-sub"
                    >Controls transfer speed. FLAC at CD quality ≈ 700–1400
                    kbps.</span
                  >
                </div>
                <input
                  class="form-input sq-custom-input"
                  type="number"
                  min="1"
                  placeholder="no limit"
                  bind:value={sqMaxBitrate}
                  disabled={sqSaving}
                />
              </div>
              <div class="sq-custom-row">
                <div class="sq-custom-info">
                  <span class="sq-custom-label"
                    >Max sample rate <code>Hz</code></span
                  >
                  <span class="sq-custom-sub"
                    >Frequency ceiling. 44100 = CD, 96000 = hi-res, 192000 =
                    studio.</span
                  >
                </div>
                <input
              ...
              <div class="sq-custom-row">
                <div class="sq-custom-info">
                  <span class="sq-custom-label"
                    >Max bit depth <code>bits</code></span
                  >
                  <span class="sq-custom-sub"
                    >Dynamic range. 16 = CD standard, 24 or 32 = hi-res.</span
                  >
                </div>
                <input
                  class="form-input sq-custom-input"
          ...
          <div class="sq-transcode-row">
            <div class="sq-custom-info">
              ...
              <span class="sq-custom-sub"
                >Re-encode on the server before delivery. Enables real bitrate
                reduction. Requires ffmpeg. Seeking may be limited.</span
              >
            </div>
            <select
              class="form-input sq-transcode-select"
              bind:value={sqTranscodeFormat}
              disabled={sqSaving}
            >
        ...
        {:else if sqTab === "wifi"}
          <p class="sq-tab-note">
            Overrides Default when your device is on Wi-Fi. Leave at "No Limit"
            to simply use your Default setting.
          </p>
          <div class="sq-presets">
            {#each SQ_PRESETS as p (p.id)}
              <button
                class="sq-preset"
                class:active={wifiPresetId === p.id}
                on:click={() => applyPreset("wifi", p)}
                disabled={sqSaving}
              >
                <div class="sq-preset-icon">
                  {#if p.id === "unlimited"}
                ...
                <span class="sq-preset-name"
                  >{p.id === "unlimited" ? "Use Default" : p.name}</span
                >
                <span class="sq-preset-desc"
                  >{p.id === "unlimited"
                    ? "Follow Default setting"
                    : p.desc}</span
                >
                <span class="sq-preset-detail"
                  >{p.id === "unlimited"
                    ? "No Wi-Fi override applied"
                    : p.wifiDesc}</span
                >
              </button>
            {/each}
          ...
          <button
            class="sq-custom-btn"
            class:open={sqWifiShowCustom || wifiPresetId === "custom"}
            on:click={() => toggleCustom("wifi")}
            disabled={sqSaving}
          >
            <svg
              width="13"
            ...
            <div class="sq-custom-form">
              <div class="sq-custom-row">
                <div class="sq-custom-info">
                  ...
                  >
                  <span class="sq-custom-sub"
                    >Leave blank to inherit from Default.</span
                  >
                </div>
                <input
                  class="form-input sq-custom-input"
              ...
              <div class="sq-custom-row">
                ...
                <input
                  ...
                  min="1"
                  placeholder="inherit default"
                  bind:value={sqWifiMaxSampleRate}
                  disabled={sqSaving}
                />
              </div>
              <div class="sq-custom-row">
          ...
          <div class="sq-transcode-row">
            <div class="sq-custom-info">
              <span class="sq-custom-label">Transcode format</span>
              <span class="sq-custom-sub"
                >Override transcode format on Wi-Fi. Null inherits from Default.</span
              >
            </div>
            <select
              class="form-input sq-transcode-select"
              bind:value={sqWifiTranscodeFormat}
              disabled={sqSaving}
            >
        ...
        {:else if sqTab === "mobile"}
          <p class="sq-tab-note">
            Overrides Default on cellular connections. Set "Data Saver" here to
            protect your mobile data plan.
          </p>
          <div class="sq-presets">
            {#each SQ_PRESETS as p (p.id)}
              <button
                class="sq-preset"
                class:active={mobilePresetId === p.id}
                on:click={() => applyPreset("mobile", p)}
                disabled={sqSaving}
              >
                <div class="sq-preset-icon">
                  {#if p.id === "unlimited"}
          ...
          <button
            class="sq-custom-btn"
            class:open={sqMobiShowCustom || mobilePresetId === "custom"}
            on:click={() => toggleCustom("mobile")}
            disabled={sqSaving}
          >
            <svg
              width="13"
            ...
            <div class="sq-custom-form">
              ...
              <div class="sq-custom-row">
                ...
                <input
                  ...
                  min="1"
                  placeholder="inherit default"
                  bind:value={sqMobileMaxSampleRate}
                  disabled={sqSaving}
                />
              </div>
              <div class="sq-custom-row">
                ...
                <input
                  ...
                  min="1"
                  placeholder="inherit default"
                  bind:value={sqMobileMaxBitDepth}
                  disabled={sqSaving}
                />
              </div>
            </div>
          ...
          <div class="sq-transcode-row">
            <div class="sq-custom-info">
              <span class="sq-custom-label">Transcode format</span>
              <span class="sq-custom-sub"
                >Override transcode format on mobile data. Null inherits from
                Default.</span
              >
            </div>
            <select
              class="form-input sq-transcode-select"
              bind:value={sqMobileTranscodeFormat}
              disabled={sqSaving}
            >
              <option value={null}>Inherit default</option>
              <option value="mp3">MP3</option>
        ...
        {#if sqSuccess}<p class="msg msg--ok">
            Streaming preferences saved.
          </p>{/if}

        <div>
          <button
            class="btn-primary"
            on:click={saveStreamingPrefs}
            disabled={sqSaving}
          >
    ...
    <section id="playback" class="card">
      ...
      <div class="setting-row" style="border-top:none;padding-top:0">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            When the queue ends, automatically add similar tracks based on what
      ...
      <div class="setting-row">
        ...
        <button
          ...
          title={$visualizerButtonEnabled
            ? "Disable visualizer button"
      ...
      <div class="setting-row">
        ...
        <button
          ...
          title={$listenAlongEnabled
            ? "Disable Listen Along button"
            : "Enable Listen Along button"}
        >
          <span class="toggle-knob"></span>
        </button>
      </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            Waveform shows a pre-generated audio waveform (falls back to
            client-side). Squiggle shows an animated sine-wave line. Line shows
            a plain flat bar.
          </span>
        </div>
        <div class="mode-toggle">
          ...
          >
            ...
            >
              <path
                d="M1 7 C3 2, 6 2, 8 7 S13 12, 15 7 S20 2, 22 7 S27 12, 29 7"
              />
            </svg>
            Squiggle
          </button>
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            When shuffle is on, spread tracks by artist so the same artist never
            plays back-to-back, and move recently played tracks toward the end
            of the order.
          </span>
        </div>
        <button
          ...
          title={$smartShuffleEnabled
            ? "Disable smart shuffle"
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            Normalize track loudness using ReplayGain metadata so every track
            plays at a consistent volume.
          </span>
        </div>
        <button
          class="toggle-btn"
          ...
          title={$replayGainEnabled
            ? "Disable auto volume leveling"
            : "Enable auto volume leveling"}
        >
          <span class="toggle-knob"></span>
        </button>
      </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            Schedule the next track to start the instant the current one ends —
            no silence between tracks. Only applies to 24-bit Hi-Res tracks on
            the Web Audio path. Can be combined with Crossfade.
          </span>
        </div>
        <button
          class="toggle-btn"
          class:on={$gaplessEnabled}
          role="switch"
          aria-checked={$gaplessEnabled}
          on:click={() => gaplessEnabled.set(!$gaplessEnabled)}
          title={$gaplessEnabled
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            Blend the end of one track into the start of the next using
            overlapping volume fades. Only applies to 24-bit Hi-Res tracks on
            the Web Audio path. Can be combined with Gapless Playback.
          </span>
        </div>
        <button
          class="toggle-btn"
      ...
      {#if $crossfadeEnabled}
        <div class="setting-row" style="align-items:center; gap:1rem;">
          <div class="setting-info" style="flex:1 1 auto;">
            ...
            <span class="setting-desc"
              >How many seconds the outgoing and incoming tracks overlap.</span
            >
          </div>
          <div
            style="display:flex; align-items:center; gap:0.6rem; flex-shrink:0;"
          >
            <input
              type="range"
              min="1"
            ...
            />
            <span
              style="min-width:2.5rem; text-align:right; font-size:0.85rem; color:var(--text-secondary);"
              >{$crossfadeSecs}s</span
            >
          </div>
        </div>
        ...
        <div class="setting-row" style="align-items:center; gap:1rem;">
          <div class="setting-info" style="flex:1 1 auto;">
            <span class="setting-name">Crossfade Start</span>
            <span class="setting-desc"
              >How many seconds before the end of the current track the
              crossfade begins.</span
            >
          </div>
          <div
            style="display:flex; align-items:center; gap:0.6rem; flex-shrink:0;"
          >
            <input
              ...
              max="20"
              step="0.5"
              value={$crossfadeStartSecs}
              on:input={(e) =>
                crossfadeStartSecs.set(
                  parseFloat((e.target as HTMLInputElement).value),
                )}
              style="width:120px; accent-color:var(--accent);"
            />
            <span
              style="min-width:2.5rem; text-align:right; font-size:0.85rem; color:var(--text-secondary);"
              >{$crossfadeStartSecs}s</span
            >
          </div>
        </div>

        <div class="setting-row setting-row--col crossfade-curve-row">
          <div class="crossfade-curve-header">
            <div class="setting-info">
              <span class="setting-name">Crossfade Curve Editor</span>
              <span class="setting-desc">
                Both curves are shown in one graph so you can tune overlap at a
                glance.
              </span>
            </div>
            <button
              class="btn-secondary btn-sm"
              on:click={resetCrossfadeCurves}
              type="button"
            >
              Reset Curves
            </button>
          ...
          <CrossfadeOverlapEditor
            outValues={$crossfadeOutCurve}
            inValues={$crossfadeInCurve}
            on:change={(e) => {
              crossfadeOutCurve.set(e.detail.outValues);
              crossfadeInCurve.set(e.detail.inValues);
            }}
          />
        </div>
      {/if}
      ...
      {#if isTauri()}
        <div class="setting-row">
          <div class="setting-info">
            ...
            <span class="setting-desc">
              Show what you're listening to in your Discord status. Requires
    ...
    <section id="sleep" class="card">
      ...
      <p class="setting-desc" style="margin:0 0 16px">
        Choose which player types show a sleep timer button in their playback
        bar.
      </p>
      <div class="sleep-grid">
        <div class="sleep-card">
          ...
          <button
            ...
            title={$musicSleepTimerEnabled
              ? "Disable music sleep timer"
              : "Enable music sleep timer"}
            ><span class="toggle-knob"></span></button
          >
        </div>
        <div class="sleep-card">
          ...
          <button
            ...
            title={$audiobookSleepTimerEnabled
              ? "Disable audiobook sleep timer"
              : "Enable audiobook sleep timer"}
            ><span class="toggle-knob"></span></button
          >
        </div>
        <div class="sleep-card">
          ...
          <button
            ...
            title={$podcastSleepTimerEnabled
              ? "Disable podcast sleep timer"
              : "Enable podcast sleep timer"}
            ><span class="toggle-knob"></span></button
          >
        </div>
      </div>
          ...
          <div class="setting-row" style="border-top:none;padding-top:0">
            <div class="setting-info">
              <span class="setting-name"
                >Connected as <strong>{scrobble.lastfm_username}</strong></span
              >
              <span class="setting-desc"
                >Scrobbles and now-playing notifications will be sent to
                Last.fm.</span
              >
            </div>
            <div style="display:flex;gap:8px;align-items:center;flex-shrink:0">
              <button
                ...
                on:click={() =>
                  toggleScrobble("lastfm", !scrobble.lastfm_enabled)}
                title={scrobble.lastfm_enabled
        ...
        {#if scrobble.lb_connected}
          <div class="setting-row" style="border-top:none;padding-top:0">
            <div class="setting-info">
              ...
              <span class="setting-desc"
                >Listens and now-playing notifications will be sent to
                ListenBrainz.</span
              >
            </div>
            <div style="display:flex;gap:8px;align-items:center;flex-shrink:0">
              <button
                ...
                title={scrobble.lb_enabled
                  ? "Disable ListenBrainz scrobbling"
        ...
        {:else}
          ...
          >
            <input
              ...
              bind:value={lbToken}
              on:keydown={(e) => {
                if (e.key === "Enter") saveLBToken();
              }}
            />
            <p class="setting-desc" style="margin:0">
              Find your token at <a
                href="https://listenbrainz.org/profile/"
                target="_blank"
                rel="noopener">listenbrainz.org/profile</a
              >.
            </p>
            {#if lbError}<p class="msg msg--error">{lbError}</p>{/if}
            {#if lbSuccess}<p class="msg msg--ok">
                ListenBrainz token saved.
              </p>{/if}
            <button
              class="btn-primary"
              style="align-self:flex-start"
              disabled={lbSaving}
              on:click={saveLBToken}
            >
    ...
    <section id="devices" class="card">
      ...
      <div class="setting-row" style="border-top:none;padding-top:0">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            When enabled, only one device can play at a time. Switching devices
            pauses all others. Disable to stream different tracks from multiple
      ...
      {#if sinkIdSupported}
        <div class="setting-row setting-row--col" style="margin-top:4px">
          <div class="setting-info">
            ...
            <span class="setting-desc">
              Route music to a different audio output — Bluetooth speakers,
              HDMI, USB DAC, etc. Only outputs paired to this computer appear
              here.
            </span>
          </div>
          <div
          ...
          >
            ...
            <button
              ...
              title="Refresh audio output list">Refresh</button
            >
          </div>
          {#if audioOutputError}<p class="msg msg--error">
              {audioOutputError}
            </p>{/if}
        </div>
      {/if}
      ...
      {#if $castState !== "unavailable"}
        <div class="setting-row" style="margin-top:4px">
          <div class="setting-info">
            ...
            <span class="setting-desc">
              {#if $castState === "connected"}
                Casting to <strong>{$castDeviceName}</strong>. Music plays on
                the remote device; stop casting to return to local playback.
              {:else}
                Stream music to a Chromecast, smart TV, or any Cast-enabled
      ...
      <div class="setting-row setting-row--col" style="margin-top:4px">
        <div class="setting-info">
          ...
          <span class="setting-desc"
            >All devices currently registered under your account. Sessions
            expire after 90 s of inactivity.</span
          >
        </div>
        ...
        {:else}
          <div class="device-list">
            {#each $activeDevices as dev (dev.id)}
              <div
                class="device-card"
                class:device-card--active={dev.is_active}
              >
                ...
                <div class="device-card-info">
                  <span class="device-card-name">
                    {dev.name}
                    {#if dev.id === deviceId}<span class="device-card-badge"
                        >This device</span
                      >{/if}
                    {#if dev.is_active}<span
                        class="device-card-badge device-card-badge--active"
                        >Active</span
                      >{/if}
                  </span>
                  ...
                  {:else}
                    <span class="device-card-track" style="color:var(--text-2)"
                      >Idle</span
                    >
                  {/if}
                </div>
    ...
    <section id="appearance" class="card">
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc"
            >Choose between dark and light interface</span
          >
        </div>
        <div class="mode-toggle">
          <button
      ...
      <div class="setting-row setting-row--col">
        <div class="setting-info">
          ...
          <span class="setting-desc"
            >Used for highlights, active states, and interactive elements</span
          >
        </div>
        <div class="palette">
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <span class="setting-desc"
            >What to show below the track title in the player bar</span
          >
        </div>
        <div class="mode-toggle">
          <button
    ...
    <section id="downloads" class="card">
      ...
      <div class="setting-row" style="border-top:none;padding-top:0">
        <div class="setting-info">
          ...
          <span class="setting-desc">
            Automatically download a track for offline playback when you add it
            to your favorites.
          </span>
        </div>
        <button
          ...
          title={$autoDownloadFavorites
            ? "Disable auto-download favorites"
            : "Enable auto-download favorites"}
        >
          <span class="toggle-knob"></span>
        </button>
      </div>
      ...
      <div class="dl-summary">
        <span class="dl-stat"
          >{doneCount + abDoneCount} file{doneCount + abDoneCount !== 1
            ? "s"
            : ""} · {formatBytes(totalSizeBytes + abTotalSizeBytes)}</span
        >
        {#if storageEst}
          <span class="dl-stat dl-stat--muted">
            {formatBytes(storageEst.usage ?? 0)} used{#if storageEst.quota}
              of {formatBytes(storageEst.quota)}{/if}
          </span>
        {/if}
      </div>
          ...
          {#each activeEntries as entry (entry.trackId)}
            <div class="dl-active-item">
              <div class="dl-active-info">
                ...
                <span class="dl-active-meta"
                  >{entry.albumName || entry.artistName || ""} · {entry.progress}%</span
                >
              </div>
      ...
      {#if errorEntries.length > 0}
        <div class="dl-errors">
          <h3 class="dl-subhead dl-subhead--warn">
          ...
          {#each errorEntries as entry (entry.trackId)}
            <div class="dl-error-item">
              ...
              {#if entry.artistName}<span class="dl-error-artist"
                  >{entry.artistName}</span
                >{/if}
              <span class="dl-error-msg">{entry.error || "Unknown error"}</span>
              <button
                class="btn-ghost"
                style="font-size:11px;padding:2px 8px"
                on:click={() => retryDownload(entry)}>Retry</button
              >
              <button
                class="btn-ghost"
                style="font-size:11px;padding:2px 8px"
                on:click={() => deleteDownload(entry.trackId)}>Dismiss</button
              >
            </div>
          {/each}
        </div>
      ...
      {#if doneCount > 0}
        <div class="dl-section-head">
          ...
          <span class="dl-section-stat"
            >{doneCount} track{doneCount !== 1 ? "s" : ""} · {formatBytes(
              totalSizeBytes,
            )}</span
          >
        </div>
        ...
        {#each albumGroups as [albumName, tracks] (albumName)}
          <div class="dl-album-group">
            <div class="dl-album-header-row">
              <button
                class="dl-album-header"
                on:click={() => toggleAlbumGroup(albumName)}
              >
                <svg
                  class="dl-chevron"
                  class:expanded={expandedAlbums.has(albumName)}
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
                  fill="none"
                ...
                >
                <span class="dl-album-count"
                  >{tracks.length} · {formatBytes(
                    tracks.reduce((s, e) => s + e.sizeBytes, 0),
                  )}</span
                >
              </button>
              <button
                class="dl-remove-btn dl-remove-btn--album"
                on:click={() => deleteAlbumGroup(tracks)}
                ...
                {#each tracks as entry (entry.trackId)}
                  <li class="dl-track-item">
                    <div class="dl-track-info">
                      ...
                      <span class="dl-track-meta"
                        >{entry.artistName || "—"} · {formatBytes(
                          entry.sizeBytes,
                        )}</span
                      >
                    </div>
                    <button
                      class="dl-remove-btn"
                      on:click={() => deleteDownload(entry.trackId)}
        ...
        {#if filteredDone.length === 0 && dlSearch.trim()}
          <p class="msg" style="color:var(--text-2)">
            No downloads match "{dlSearch}"
          </p>
        {/if}
      {:else if activeEntries.length === 0 && errorEntries.length === 0 && abDoneCount === 0}
        <p class="msg" style="color:var(--text-2)">
          No tracks downloaded yet. Right-click any track and choose "Download
          offline".
        </p>
      {/if}
      ...
      {#if abDoneCount > 0}
        <div
          class="dl-section-head"
          style="margin-top: {doneCount > 0 ? '8px' : '0'}"
        >
          <h3 class="dl-subhead">Audiobooks</h3>
          <span class="dl-section-stat"
            >{audiobookGroups.length} book{audiobookGroups.length !== 1
              ? "s"
              : ""} · {formatBytes(abTotalSizeBytes)}</span
          >
        </div>
        ...
        {#each audiobookGroups as [abKey, chapters] (abKey)}
          ...
          <div class="dl-album-group">
            <div class="dl-album-header-row">
              <button
                class="dl-album-header"
                on:click={() => toggleABGroup(abKey)}
              >
                <svg
                  class="dl-chevron"
                  class:expanded={expandedABGroups.has(abKey)}
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
                  fill="none"
                ...
                <span class="dl-album-count"
                  >{bookAuthor ? bookAuthor + " · " : ""}{chapters.length} ch · {formatBytes(
                    chapters.reduce((s, e) => s + e.sizeBytes, 0),
                  )}</span
                >
              </button>
              <button
                class="dl-remove-btn dl-remove-btn--album"
                on:click={() => deleteAlbumGroup(chapters)}
                title="Delete audiobook downloads"
              >
                <svg
                  width="12"
                  height="12"
        ...
        <div style="padding-top:8px;display:flex;align-items:center;gap:8px">
          {#if confirmDeleteAll}
            <span style="font-size:13px;color:var(--text-2)"
              >Delete all downloads?</span
            >
            <button class="btn-danger" on:click={handleDeleteAll}
              >Confirm</button
            >
            <button
              class="btn-ghost"
              on:click={() => (confirmDeleteAll = false)}>Cancel</button
            >
          {:else}
            <button
              class="btn-danger"
...
<style>
  ...
  .swatch:hover {
    transform: scale(1.1);
    box-shadow: 0 0 0 4px var(--swatch-glow);
  }
  .swatch.selected {
    border-color: white;
  ...
  .sq-preset.active .sq-preset-icon {
    color: var(--accent);
  }
  ...
  .sq-preset.active .sq-preset-name {
    color: var(--accent);
  }
  ...
  .sq-custom-btn:hover:not(:disabled) {
    border-color: var(--accent);
    color: var(--text);
  }
  .sq-custom-btn.open {
