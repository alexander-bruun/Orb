...
//
// After the weighted sum is computed a small diversity bonus is added to
// cross-artist and cross-album pairs so that recommendation queues stay
// varied rather than clustering within a single artist or album.
package similarity
...
const (
	...
	// Diversity bonus — additive reward applied after the weighted sum to
	// discourage recommendation queues that cluster within one album / artist.
	// The final score is still clamped to [0, 1].
	diversityBonusCrossArtist = 0.04 // different primary artist
	diversityBonusCrossAlbum  = 0.02 // same artist, but different album
...
// ComputeForTracks is an incremental variant of ComputeAll. It loads all track
// data but only computes pairs where at least one side is in newTrackIDs.
// Results are upserted without clearing existing similarity data, so pairs
// between unchanged tracks remain intact.
//
// Complexity: O(k × n) where k = len(newTrackIDs) and n = total tracks,
// compared to O(n²) for a full recompute.
func ComputeForTracks(ctx context.Context, db *store.Store, newTrackIDs []string) error {
	if len(newTrackIDs) == 0 {
		return nil
	}
	newSet := make(map[string]bool, len(newTrackIDs))
	for _, id := range newTrackIDs {
		newSet[id] = true
	}

	slog.Info("loading track data for incremental similarity computation", "new_tracks", len(newTrackIDs))
	...
	slog.Info("computing incremental pairwise similarity", "total_tracks", len(vecs))

	topK := make(map[string][]similarPair)
	for i := 0; i < len(vecs); i++ {
		for j := i + 1; j < len(vecs); j++ {
			// Only evaluate pairs that include at least one new track.
			if !newSet[vecs[i].ID] && !newSet[vecs[j].ID] {
				continue
			}
			a, b := &vecs[i], &vecs[j]
			score := computeScore(a, b, genreIDF, related, coFeatArtists, coPlay)
	...
	slog.Info("upserting incremental similarity pairs", "pairs", len(rows))
	if err := db.UpsertSimilarityIncremental(ctx, rows); err != nil {
		return fmt.Errorf("upsert incremental similarity: %w", err)
	}
	return nil
}
...
func computeScore(
	...
	// Diversity bonus: lift cross-artist / cross-album pairs so that
	// the recommendation queue doesn't cluster within one album or artist.
	if a.ArtistID != b.ArtistID {
		score += diversityBonusCrossArtist
	} else if a.AlbumID != b.AlbumID {
		score += diversityBonusCrossAlbum
	}
